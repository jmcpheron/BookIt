...
$(document).ready(function() {
  var gcal = "http://www.google.com/calendar/feeds/jmcpheron%40sce.edu/private-7bf916006d32317a5639503aad5fd72c/basic";
  var source = $.fullCalendar.gcalFeed(gcal, {className: 'noms', editable: true, id: 'g'});

  $("button#gcal").click( function(){
...
firstHour: <?echo $start_hour;?>,
  events: [
<?
$open_appts = getOpenAppointments($sou, $srole, $start_day, $end_day, null, $ou, $role);
...
if($open_appts){
  ...
  foreach($open_appts as $item){
    if( ($last_start_slot != $item['start_time']) or ($last_end_slot != $item['end_time']) ){
      $events_string .= drawBlockByBid($item['bid'], "add_participant.php?bid=".$item['bid']."&".$query_string, null, 'darkgreen');
    }
    $last_start_slot = $item['start_time'];
    $last_end_slot = $item['end_time'];
  }
}
//Trim last comma
...
echo "</div>";
?>
<button class="btn success" id="gcal">Import External Calendar</button>
<hr />
